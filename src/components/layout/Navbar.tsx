...
const navLinks = [
  ...
  { href: 'https://canva.link/etz1jmej84o4xcv', label: 'Resume', external: true },
];
            ...
            <div className="relative">
              <div className="flex items-center space-x-1 bg-gradient-to-r from-gray-100/50 to-white/30 dark:from-gray-800/50 dark:to-gray-900/30 backdrop-blur-sm rounded-full pl-6 pr-3 py-1.5 border border-gray-200/50 dark:border-gray-700/50 shadow-sm">
                ...
                {navLinks.map((link) => {
                  ...
                  const linkClasses = `relative px-3 py-1.5 text-sm font-medium transition-all duration-200 rounded-full
                    ${isActive
                      ? 'text-white'
                ...
                <div className="ml-2">
                  ...
                  >
                    ...
                    <motion.div
                      className={`absolute w-5 h-5 rounded-full z-10 shadow-md flex items-center justify-center ${isDarkMode
            ...
            <div className="container mx-auto py-6 px-5">
              <div className="grid grid-cols-2 gap-2">
                {navLinks.map((link, idx) => {
                  ...
                  const subtitles: Record<number, string> = { 0: 'Main', 1: 'My expertise', 2: 'My story', 3: 'My work', 4: 'Get in touch', 5: 'View CV' };
                  const mobileContent = (
                    <>
                      {isActive && (
                        <motion.div
                    ...
                    >
                      {isExternal ? (
                        <a
                          ...
                          className={`block p-4 rounded-xl transition-all relative overflow-hidden hover:bg-gray-100/70 dark:hover:bg-gray-800/70`}
                        >
                          {mobileContent}
                        </a>
                      ) : (
                        <Link
                          href={link.href}
                          onClick={() => setIsMenuOpen(false)}
                          className={`block p-4 rounded-xl transition-all relative overflow-hidden ${isActive
                            ? 'shadow-md'
